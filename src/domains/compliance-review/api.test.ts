      ...
      claims: [
        {
          ...
          ruleFiles: [
            {
              ruleFileId: 1,
              ruleFileUrl: 'http://localhost:8080/files/rules/rule.pdf',
              ruleFileTitle: '금융광고심의규정.pdf',
...
describe('fetchComplianceReviewDetail', () => {
  it('gets compliance review detail by compliance id', async () => {
    ...
    const fetcher: typeof fetch = async (url, init) => {
      ...
      return new Response(JSON.stringify({ comScore: 85, claims: [] }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    }
